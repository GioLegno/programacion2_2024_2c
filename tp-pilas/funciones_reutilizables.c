...
bool ingresoEntero(int* n) {
    char buffer[10];  // Tamaño maximo de entrada
    bool res = true;
    int i;

    while (1) {  // Bucle para seguir pidiendo el dato hasta que sea válido
        ...
        len = strlen(buffer);  // Actualizar la longitud después de eliminar espacios
        ...
        // Comprobar si es un número válido (incluyendo negativos)
        i = 0;
        if (buffer[0] == '-' && len > 1) {
        ...
        // Si el valor es válido, procesamos el número
        if (esValido) {
            *n = atoi(buffer);
            break;  // Salir del bucle ya que se ingresó correctamente
        }
    }
